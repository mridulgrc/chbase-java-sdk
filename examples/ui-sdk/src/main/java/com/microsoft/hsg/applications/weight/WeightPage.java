...
public class WeightPage extends WebPage {
	...
	public WeightPage(final PageParameters parameters) {
		...
		thingList = new ListView("weightList", weightList) {
			protected void populateItem(ListItem item) {
				...
				item.add(new DeleteForm("delete",wi.getId(),wi.getVersion() ));
			}
		};
		add(thingList);
    }
	...
	private final class DeleteForm extends Form
	{
		public DeleteForm(String id, String thingid, String version)
		{
			super(id, new CompoundPropertyModel(new ValueMap()));
			
			HiddenField hdn_version = new HiddenField("version");
			HiddenField hdn_thingid = new HiddenField("thingID");
			hdn_thingid.add(new SimpleAttributeModifier("value", thingid));
			hdn_version.add(new SimpleAttributeModifier("value", version));
    		add(hdn_thingid);
    		add(hdn_version);
		}
		protected void onSubmit()
    	{
			...
			String thingID = (String)map.get("thingID");
    		String version = (String)map.get("version");
    		WeightDAO dao = new WeightDAO(ConnectionFactory.getConnection());
    		dao.deleteWeight(thingID, version);
